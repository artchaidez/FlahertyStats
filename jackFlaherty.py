...
#second half minus 1st half, to show differences
results2 - results1
...
#His pitches are the names of the bars
...
#width of bars (width), very thin to fit 4 bars
...
""" THIS WORKS BABY! How columns are organized 
2018, 2019 1st half, and 2019 2nd half velo       """
...
#set the y label to MPH and range of numbers
ax.set_ylabel('MPH')
ax.set_ylim(0,100)
#Legend to identify each half 
ax.legend(loc='center')
#swt the x axis labels and total amound of bars 
ax.set_xticklabels(pitches)
ax.set_xticks(ind)
...
#lets look at his 2018 season, to compare to 2019
...
understand what this all means, we can see differences in a graph."""
...
#set the x-axis from -3 to 0
#set the y axis from 0 to 6
#label each axis, include legend and title of plot
...
#create multiple ellipses using a function - not sure where I found this
def makeEllipses(list):
    for a in list:
        ax.add_artist(a)

#cant figure out a for loop to make an ellipse of each pitch
#had to do each pitch individually, very ugly
#see the same thing as before, its still a good learning experience 
#run all lines together
...
#Average movement on 2018, 2019 1st half, 2019 2nd half
#run all lines together
#Squezzed some things together to not have to run so many lines
#Note:  using a scatter plot for this kind of data is terrible
#I also cannot decide a good graph to present this type of data
fig, ax = plt.subplots()
...
#2019 halves in ellipses. Same as above, just wanted the practice
#Run all lines together, squeezed some lines for it to run easier
fig, ax = plt.subplots()
...
results (if the ball was put in play). I divided each sum of batted ball type
...
#Rename the indexes from numbers to their batted ball names
...
#this is much better, also explains what each height means!
fig, ax = plt.subplots()
